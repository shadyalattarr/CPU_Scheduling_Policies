...
Process* select_process(priority_queue<Process*, vector<Process*>, CompareRemainingTime> * pq){
...
vector<vector<char>> run_srt(vector<Process>& all_processes,int num_processes,int last_instant){
    ...
    for(int time=0;time<=last_instant;time++){
        ...
        if(current_process_running->process_name != "null"){
            ...
            // need to check that the current process is not done to do this
            if(current_process_running->remaining_time > 0){
                // not done
            ...
            // can always do ^ 
            // if empty queue, will get it itself
            // if not empty then reordered
        }
