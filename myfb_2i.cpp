...
vector<Process> get_Processes_as_struct(vector<string> processes,int num_processes){
    ...
    for(int i = 0;i<num_processes;i++){
        ...
        curr_process.time_in_quantum = 0;
...
void serveProcess(Process* current_process_running, int time) {
    ...
    current_process_running->time_in_quantum++;
...
int get_quantum(int queue_number){
    return static_cast<int>(pow(2, queue_number));
}

vector<vector<char>> run_fb2i(vector<Process>& all_processes,int num_processes,int last_instant){
    ...
    for(int time=0;time<=last_instant;time++){
        ...
        //cout << "time: " << time << endl;
        //cout << "current_process: " << current_process_running->process_name << " quantum " << get_quantum(current_process_running->queue_number) << endl;
        ...
        if(current_process_running->process_name != "null"){ // there is a currently running process
            ...
            if(current_process_running->time_served_so_far < current_process_running->service_time){
                // still not done
                // check quantum
                quantum = get_quantum(current_process_running->queue_number);
                //cout << "quantum rn for curr p " << current_process_running->process_name << quantum <<endl;
                //cout << "time spent in quantum " << current_process_running->time_in_quantum << endl;
                //quantum = get_quantum(current_process_running->queue_number);
                if(current_process_running->time_in_quantum%quantum == 0){
                    // it's time is done
                    ...
                    if(is_there_process_ready(queues)){
                        ...
                        //cout << "PREEMPT " << current_process_running->process_name << endl;
                        prevp->queue_number++;
                        //cout << "its new queue: " << current_process_running->queue_number << endl;
                        prevp->time_in_quantum = 0; // for next up
                        queues[prevp->queue_number].push(prevp); // preempt process
                        
                        //cout << "new process chosen: " << current_process_running->process_name << " queue number: " << current_process_running->queue_number << endl;
                        //quantum = get_quantum(current_process_running->queue_number);
                        ...
                        // nn to push again cuz we dont change anything ig
            ...
            // update trace?
                ...
                current_process_running = select_process(queues);
                //quantum = get_quantum(current_process_running->queue_number);
                current_process_running->time_in_quantum = 0;
                serveProcess(current_process_running,time);
...
int main(int argc, char const *argv[]) {
    ...
    vector<vector<char>> trace = run_fb2i(all_processes,num_processes,last_instant); // we dont need last_instant?
